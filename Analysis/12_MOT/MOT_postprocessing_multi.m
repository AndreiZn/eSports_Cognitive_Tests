function [PP_CFG, PP_DATA] = MOT_postprocessing_multi(PP_CFG, PP_DATA, CFG_array, DATA_array, test_idx)
...
if ~flag_all_data_valid
    ...
    for idx = 1:size(DATA_pro, 1)
        DATA_pro_concat = [DATA_pro_concat; ...
                           DATA_pro(idx).tests{test_idx}.num_target, DATA_pro(idx).tests{test_idx}.accuracy];
    end
    DATA_pro_concat(:, 2) = DATA_pro_concat(:, 2) * 100;

    for idx = 1:size(DATA_non, 1)
        DATA_non_concat = [DATA_non_concat; ...
                           DATA_non(idx).tests{test_idx}.num_target, DATA_non(idx).tests{test_idx}.accuracy];
    end
	...
	title('Multi-Obiect Tracking Test');
